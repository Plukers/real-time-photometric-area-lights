...
[<ReflectedDefinition>][<Inline>]
let private sampleIrr (t2w : M33d) (addr : int) forward up area (p : V3d) = 
                    ...
                    for iIdx in iAddr .. Config.Light.MAX_PATCH_IDX_BUFFER_SIZE_PER_LIGHT .. (iAddr + uniform.LNumPatchIndices.[addr] - 1) do
                        ...
                        if clippedVc <> 0 then
                            ...
                            if not insideLightPlane then
                                ...
                                let forward = uniform.LForwards.[addr]
                                let up = uniform.LUps.[addr]
                                let area = uniform.LAreas.[addr]

                                let mutable vc = clippedVc
                                let mutable offset = 0
                                if caseOffset <> CASE_CORNER_OFFSET then 
                                    vertices.[0] <- closestPointClamped |> Vec.normalize
                                    funVal.[0]   <- sampleIrr t2w addr forward up area closestPointClamped
                                    vc <- vc + 1 
                                    offset <- 1

                                for i in 0 .. clippedVc - 1 do 
                                    vertices.[i + offset] <- clippedVa.[(v1Idx + i) % clippedVc]
                                    funVal.[i + offset]   <- sampleIrr t2w addr forward up area clippedVa.[(v1Idx + i) % clippedVc]
                    ...
                    for iIdx in iAddr .. Config.Light.MAX_PATCH_IDX_BUFFER_SIZE_PER_LIGHT .. (iAddr + uniform.LNumPatchIndices.[addr] - 1) do
                        ...
                        if abs dotOut > 1e-3 then
                            ...
                            if clippedVc <> 0 then
                                ...
                                //let (patchIllumination, weight, area) = computeIlluminationCheap delEdgeData vertices caseOffset
                                let (patchIllumination, weight, area) = computeIllumination delEdgeData delMetaData vertices caseOffset
            ...
            if clippedVc <> 0 then
                ...
                if not insideLightPlane then
                    ...
                    let slow () =
                        ...
                        let forward = lForwards.[addr]
                        let up = lUps.[addr]
                        let area = lAreas.[addr]
                        ...
                        for i in 0 .. funVal.Length - 1 do
                            funVal.[i] <- V2d( 1.0, funVal.[i].Y)
                        ...
                        printfn "-------- SLOW ----------"
                        printfn "  PatchIllumination: %A" (patchIllumination, weightSum, areaSum)

                        (vertices, delEdgeData)
                    ...
                    let fast () =
                        ...
                        let mutable delEdgeData_F = QUAD_DATA.initEdgeDataSlice CASE_CORNER_OFFSET 
                        if caseOffset = CASE_EDGE_OFFSET then
                            delEdgeData_F <- QUAD_DATA.initEdgeDataSlice CASE_EDGE_OFFSET 
                        else if caseOffset = CASE_INSIDE_OFFSET then
                            delEdgeData_F <- QUAD_DATA.initEdgeDataSlice CASE_INSIDE_OFFSET 
                          
                        //let delFaceData = QUAD_DATA.getInitFaceData caseOffset
                        let delFaceData_F = Arr<N<MAX_FACES_HALF>, V4i>()
                        for i in 0 .. MAX_FACES_HALF - 1 do
                            delFaceData_F.[i] <- QUAD_DATA.ALL.FACES.[MAX_FACES_HALF * caseOffset + i]
                                    
                        let mutable delMetaData_F = QUAD_DATA.getInitMetaData caseOffset
                        ...
                        let stack_F = Arr<N<MAX_EDGES>, int>()
                        let mutable SP_F = -1
                                    
                        for i in 0 .. MAX_EDGES - 1 do
                            if edgeIsInside delMetaData_F i then
                                SP_F <- SP_F + 1
                                stack_F.[SP_F] <- i
                                delMetaData_F <- markEdge delMetaData_F i
                        ...
                        while SP >= 0 do
                            ...
                            if notLD then
                                ...
                                let flipResult =  flipEdge delEdgeData_F delMetaData_F delFaceData_F stack_F SP_F eId
                        ...
                        for f in 0 .. MAX_FACES - 1 do
                            ...
                            if not (faceIsEmpty delFaceData_F f) then
                                let area = computeSphericalExcess (vertices.[readFaceVertexId delFaceData_F f 0].XYZ |> Vec.normalize) (vertices.[readFaceVertexId delFaceData_F f 1].XYZ |> Vec.normalize) (vertices.[readFaceVertexId delFaceData_F f 2].XYZ |> Vec.normalize)

                                patchIllumination <- patchIllumination + area * (vertices.[readFaceVertexId delFaceData_F f 0].W + vertices.[readFaceVertexId delFaceData_F f 1].W + vertices.[readFaceVertexId delFaceData_F f 2].W) / 3.0
                                weightSum <- weightSum + area * (vertices.[readFaceVertexId delFaceData_F f 0].Z + vertices.[readFaceVertexId delFaceData_F f 1].Z + vertices.[readFaceVertexId delFaceData_F f 2].Z) / 3.0
                                areaSum <- areaSum + area
                        
                        printfn "-------- QUICK ----------"

                        printfn "Case: %s" (match caseOffset with |CASE_CORNER_OFFSET -> "Corner" |CASE_EDGE_OFFSET -> "Edge" |_ -> "Inside")

                        printfn "  PatchIllumination: %A" (patchIllumination, weightSum, areaSum)


                        let patchIllumination = computeIllumination delEdgeData delMetaData vertices offset
