...
%                 plot(x1,y1,'LineWidth', 3)
    ...
    function [errorReportEntry, ssimReportEntry, errorPerSolidAngle] = EvaluateLight(light)
        ...
        for a = 1:size(Approximations,1)
            ...
            nmsError = msError / (mean(GroundTruth(:)) * mean(Approx(:)));
            ...
            positive = ediff;
            positive(ediff < 0) = 0;
            positive = filloutliers(positive,'nearest','mean','ThresholdFactor',medianDist);
            maxError = max(max(positive)); %% USE
            ...
            negative = ediff;
            negative(ediff > 0) = 0;
            negative = filloutliers(negative,'nearest','mean','ThresholdFactor',medianDist);
            minError = min(min(negative)); 
            ...
            errorReportEntry(eri) = {num2str(nmsError)};
            eri = eri + 1;
        ...
        %% Generate comparable error images

%         comparePath = fullfile(evalPath, 'comparable_error_images');
%         if exist(comparePath, 'dir')
%             cmd_rmdir(comparePath);
%         end
%         mkdir(comparePath);
% 
%         for a = 1:size(Approximations,1)
